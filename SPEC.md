...
> **v0.9 變更：** 原設計只生成 `*Queries` method + `BulkQuerier` interface。經 TransTaiwan 專案實際試用發現，當使用者開啟 `emit_interface: true` 並透過 `Querier` interface 操作 DB 時，plugin 生成的 method 不在 `Querier` 上，call site 必須做 type assertion 才能呼叫。
>
