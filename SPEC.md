...
> **v1.0 變更：** 根據實際專案 SQL 調查，確立以 `VALUES (UNNEST(...))` 為主要支援格式。原 SPEC 範例使用的 `SELECT * FROM UNNEST(...)` 格式在實際專案中未使用，列為後續支援項目。

- 對應 SQL 使用 `INSERT ... VALUES (UNNEST(...)) ON CONFLICT DO UPDATE` 語法
- 同 FR-2，生成 adapter 負責欄位拆解
- Adapter 的 Item struct、轉換邏輯、template 與 UPDATE 完全相同，差異僅在解析階段

**支援的 SQL 格式（`VALUES (UNNEST(...))`）：**

```sql
-- @bulk upsert
-- name: UpsertStopTable :exec
INSERT INTO tra_stop_table (train_id, station, stop_sequence, arrival, departure)
VALUES (
    UNNEST($1::int[]),
    UNNEST($2::text[]),
    UNNEST($3::smallint[]),
...
SET stop_sequence = EXCLUDED.stop_sequence, arrival = EXCLUDED.arrival, departure = EXCLUDED.departure;
```
...
|---|---|---|
| UNNEST 位置 | `FROM (SELECT UNNEST($N::type[]) AS alias)` | `VALUES (UNNEST($N::type[]))` |
| 有 AS alias | ✅ 有 | ❌ 無 |
| 欄位名稱來源 | UNNEST alias（FR-7） | INSERT column list（FR-9） |
| 目標 table 來源 | `Query.Text` regex 解析（FR-8） | `Query.InsertIntoTable.Name`（sqlc 直接提供） |
| Params / Struct / 轉換 | 共用邏輯 | 共用邏輯 |

**Upsert 欄位名稱解析：** UNNEST 在 VALUES 中沒有 `AS alias`，欄位名稱改從 `INSERT INTO xxx (col1, col2, ...)` 的 column list 提取，按位置與 `$N` 參數對應（見 FR-9）。
...
- `-- @bulk upsert` → 生成 bulk Upsert adapter
...
#### FR-9　INSERT Column List 解析（Upsert 欄位名稱來源）

> **v1.0 新增：** Upsert 使用 `VALUES (UNNEST(...))` 格式時，UNNEST 沒有 `AS alias`，無法使用 FR-7 的 alias 解析。欄位名稱改從 `INSERT INTO xxx (col1, col2, ...)` 的 column list 提取。

**問題：** Upsert SQL 的 UNNEST 在 VALUES 中，沒有 `AS alias`：

```sql
INSERT INTO products (id, name, price)
VALUES (
    UNNEST($1::int[]),    -- 無 AS alias
    UNNEST($2::text[]),
    UNNEST($3::int[])
)
```

**解法：** 從 `INSERT INTO xxx (col1, col2, ...)` 提取 column list，按位置與 UNNEST 參數對應。
...
提取括號內的 column list 後，以逗號分割並 trim，得到 column names 的有序清單。

範例：`INSERT INTO products (id, name, price)` → `["id", "name", "price"]`

再與 VALUES 中的 UNNEST 按位置對應：
- 第 1 個 UNNEST（`$1`）→ `id`
- 第 2 個 UNNEST（`$2`）→ `name`
- 第 3 個 UNNEST（`$3`）→ `price`

**UNNEST 參數順序解析：** VALUES 中的 UNNEST 需提取 `$N`，用於與 column list 位置對應：

```
UNNEST\(\$(\d+)::\w+\[\]\)
```

注意此 regex 與 FR-7 不同——不要求 `AS alias` 後綴。

**判斷使用哪種解析策略：**
- `@bulk update` → FR-7（UNNEST alias）+ FR-8（UPDATE table regex）
- `@bulk upsert` → FR-9（INSERT column list）+ `Query.InsertIntoTable.Name`
...
>
> **v0.3 補充：** 經實際檢驗 sqlc 行為，確認 sqlc 不會跨 query 共用 struct——即使多個 query 的欄位完全相同，也各自生成獨立的 `XxxRow` / `XxxParams`。Plugin 的 struct 複用策略依循相同原則。
...
2. 取得每個 parameter 的欄位名稱：
   - **UPDATE**：從 `Query.Text` 解析 UNNEST alias（FR-7）——永遠需要，因為 `@param` 的 `column.name` 是 param name 而非 table column name
   - **Upsert**：從 `INSERT INTO xxx (col1, col2, ...)` 解析 column list（FR-9），按位置與 UNNEST 參數對應
...
> **v0.9 變更：** 原設計只生成 `*Queries` method + `BulkQuerier` interface。實際試用發現，當使用者開啟 `emit_interface: true` 並透過 `Querier` interface 操作 DB 時，plugin 生成的 method 不在 `Querier` 上，call site 必須做 type assertion 才能呼叫。
>
> 重新設計為三種生成模式，透過 `style` 參數控制。
...
| `style` | 生成內容 | 適用情境 |
|---|---|---|
| `function`（預設） | Item struct + standalone function（接受 `Querier`） | 使用 `emit_interface: true`，不想改現有 interface |
| `method` | Item struct + `*Queries` method | 未使用 `emit_interface`，直接操作 `*Queries` |
| `interface` | Item struct + `*Queries` method + `BulkQuerier` interface + compile-time check | 使用 `emit_interface: true`，願意改用組合 interface |

**三種模式的 Item struct 和轉換邏輯完全相同**，差別只在外層包裝。

**`style: function`（預設）— standalone function：**

```go
type BulkUpdateProductsItem struct { ... }
...
call site：`sqlc.BulkUpdateProductsBatch(ctx, qtx, items)`

選為預設的理由：耦合度最低，不改任何現有 interface 或型別宣告，與 `Querier` 直接相容。

**`style: method` — `*Queries` method：**
...
call site：`q.BulkUpdateProductsBatch(ctx, items)`（`q` 必須是 `*Queries`，不能是 `Querier` interface）

**`style: interface` — method + `BulkQuerier` interface：**
...
call site：`qtx.BulkUpdateProductsBatch(ctx, items)`（`qtx` 型別須改為 `BulkQuerier`）
...
- ~~取得目標 table：UPDATE 從 `Query.Text` 以 regex 解析（FR-8）~~ ✅
- ~~取得欄位名稱：UPDATE 從 `Query.Text` 以 regex 解析 UNNEST alias（FR-7）~~ ✅
...
- 取得目標 table：Upsert 用 `Query.InsertIntoTable.Name`
- 取得欄位名稱：Upsert 從 `INSERT INTO xxx (col1, col2, ...)` 解析 column list（FR-9）
- 輸出檔案命名：UPDATE 和 Upsert 的 adapter 合併輸出至同一個檔案 `bulk.go`（原 `bulk_update.go` 改名）
